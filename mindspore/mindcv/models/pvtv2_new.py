...
class PyramidVisionTransformerV2Impl(nn.Cell):
    r"""Pyramid Vision Transformer V2 model class, based on
    `"PVTv2: Improved Baselines with Pyramid Vision Transformer" <https://arxiv.org/abs/2106.13797>`_
...
class EnhancedFeature(nn.Cell):
    def __init__(self, in_chan, is_first=False):
        super(EnhancedFeature, self).__init__()
        self.inchan = in_chan
        self.is_first = is_first
        self.conv1 = nn.SequentialCell(
            nn.Conv2d(in_channels=2 * in_chan, out_channels=in_chan, kernel_size=3, pad_mode='same', has_bias=False),
            nn.BatchNorm2d(in_chan),
            nn.ReLU(),
        )

        self.conv2 = nn.SequentialCell(
            nn.Conv2d(in_channels=in_chan, out_channels=in_chan, kernel_size=3, pad_mode='same', has_bias=False),
            nn.BatchNorm2d(in_chan),
            nn.ReLU(),
        )

        self.conv3 = nn.SequentialCell(
            nn.Conv2d(in_channels=3 * in_chan, out_channels=in_chan, kernel_size=3, pad_mode='same', has_bias=False),
            nn.BatchNorm2d(in_chan),
            nn.ReLU(),
        )
    ...
    def construct(self, x0, x1, x2):
        w = self.sigmoid(self.conv1(self.concat((x1, x2))))
        feat_x1 = self.mul(x1, w)
        feat_x2 = self.mul(x2, w)
        x = self.conv3(self.concat((self.conv2(feat_x1 + feat_x2), x1, x2)))
        if not self.is_first:
            x = self.conv(self.concat((x0, x)))
        return x
...
def pvt_v2_b2(
    ...
    if pretrained:
        param_dict = mindspore.load_checkpoint('/pretrainmodel/pvtv2.ckpt')
        param_not_load, _ = mindspore.load_param_into_net(model, param_dict)

    return model

@register_model
def pvt_v2_b2_encoder(
        pretrained: bool = False, num_classes: int = 1000, in_channels: int = 3, **kwargs
) -> PyramidVisionTransformerV2Impl:
    """Get PVTV2-b2 model
    Refer to the base class "models.PVTv2" for more details.
    """
    default_cfg = default_cfgs["pvt_v2_b2"]
    model = PyramidVisionTransformerV2Impl(
        in_chans=in_channels, num_classes=num_classes,
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, epsilon=1e-6), depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], **kwargs)
